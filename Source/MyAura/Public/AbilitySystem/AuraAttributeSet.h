...
// 属性访问器宏（单独定义）
#define ATTRIBUTE_ACCESSORS(ClassName,AttrName) \
	GAMEPLAYATTRIBUTE_PROPERTY_GETTER(ClassName, AttrName) \
	GAMEPLAYATTRIBUTE_VALUE_GETTER(AttrName) \
	GAMEPLAYATTRIBUTE_VALUE_SETTER(AttrName) \
	GAMEPLAYATTRIBUTE_VALUE_INITTER(AttrName)
...
//为函数类型别名创建一个模板，它可以为任意静态函数类型创建别名，别名为TStaticFuncPtr。
template<class T>
...
public:
	...
	//此时函数签名是零参数，返回类型是FGameplayAttribute
	TMap<FGameplayTag,TStaticFuncPtr<FGameplayAttribute()>> TagsToAttributes;  //属性菜单中需要用到此Map
	...
	// Base Attribute
	UPROPERTY(BlueprintReadOnly,Replicatedusing = OnRep_HP,Category="Base Attributes")
